...
def main():
    ...
    # Transfer MXNB from whale to deployer
    print("[INFO] Impersonating MXNB whale...")
    send_rpc("hardhat_impersonateAccount", [MXNB_WHALE])
    
    print(f"[INFO] Transferring 50,000 MXNB to deployer...")
    tx_params = {
        "from": MXNB_WHALE,
        "to": MXNB,
        "data": calldata
    ...
    tx_result = send_rpc("eth_sendTransaction", [tx_params])
    if tx_result and "result" in tx_result:
        print(f"[SUCCESS] MXNB transfer tx: {tx_result['result']}")
    else:
        print("[WARN] MXNB transfer initiated")
    
    send_rpc("hardhat_stopImpersonatingAccount", [MXNB_WHALE])
